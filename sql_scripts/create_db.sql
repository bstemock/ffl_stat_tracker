...
begin
	select sum(case
			   when away_owner = owner1 and home_owner = owner2 and winner = "Away" and week < 15 then 1
               when away_owner = owner2 and home_owner = owner1 and winner = "Home" and week < 15 then 1
               else 0
               end) as wins,
		   sum(case
			   when away_owner = owner1 and home_owner = owner2 and winner = "Home" and week < 15 then 1
               when away_owner = owner2 and home_owner = owner1 and winner = "Away" and week < 15 then 1
               else 0
               end) as losses,
		   sum(case
		       when away_owner = owner1 and home_owner = owner2 and winner = "Tie" and week < 15 then 1
               when away_owner = owner2 and home_owner = owner1 and winner = "Tie" and week < 15 then 1
               else 0
               end) as ties,
		   sum(case
			   when away_owner = owner1 and home_owner = owner2 and winner is NULL and week < 15 then 1
               when away_owner = owner2 and home_owner = owner1 and winner is NULL and week < 15 then 1
               else 0
...
begin
    ...
    where g.year = input_year and (g.home_owner = owner or g.away_owner = owner) and week < 15
    order by week asc;
end |
...
begin
	select t.owner,
	       ...
	       sum(case
		       when t.owner = g.away_owner and winner = "Tie" and g.week < 15 then 1
               when t.owner = g.home_owner and winner = "Tie" and g.week < 15 then 1
               else 0
               end) as ties,
	       round(sum(case
					 when t.owner = g.away_owner and winner = "Away" and g.week < 15 then 1
				 ...
				 sum(case
					 when t.owner = g.away_owner and g.week < 15 and g.away_score is not NULL then 1
					 when t.owner = g.home_owner and g.week < 15 and g.home_score is not NULL then 1
					 else 0
					 end), 3) as win_pct,
	       sum(case
		       when t.owner = g.home_owner and g.winner = "Home" and g.week < 15 then 1
               else 0
               end) as home_wins,
	       sum(case
		       when t.owner = g.home_owner and g.winner = "Away" and g.week < 15 then 1
               else 0
               end) as home_losses,
	       sum(case
		       when t.owner = g.home_owner and g.winner = "Tie" and g.week < 15 then 1
               else 0
               end) as home_ties,
	       sum(case
		       when t.owner = g.away_owner and g.winner = "Away" and g.week < 15 then 1
               else 0
               end) as away_wins,
	       sum(case
		       when t.owner = g.away_owner and g.winner = "Home" and g.week < 15 then 1
               else 0
               end) as away_losses,
	       sum(case
		       when t.owner = g.away_owner and g.winner = "Tie" and g.week < 15 then 1
               else 0
               end) as away_ties,
	       sum(case
		       when t.owner = g.away_owner and winner = "Away" and g.week < 15 and t.division = (select division
																							     from teams
																							     where owner = g.home_owner and year = g.year) then 1
		       when t.owner = g.home_owner and winner = "Home" and g.week < 15 and t.division = (select division
																							     from teams
																							     where owner = g.away_owner and year = g.year) then 1
		       else 0
               end) as div_wins,
	       sum(case
		       when t.owner = g.away_owner and winner = "Home" and g.week < 15 and t.division = (select division
																							     from teams
																							     where owner = g.home_owner and year = g.year) then 1
		       when t.owner = g.home_owner and winner = "Away" and g.week < 15 and t.division = (select division
																							     from teams
																							     where owner = g.away_owner and year = g.year) then 1
		       else 0
               end) as div_losses,
	       sum(case
		       when t.owner = g.away_owner and winner = "Tie" and g.week < 15 and t.division = (select division
																							    from teams
																							    where owner = g.home_owner and year = g.year) then 1
		       when t.owner = g.home_owner and winner = "Tie" and g.week < 15 and t.division = (select division
																							    from teams
																							    where owner = g.away_owner and year = g.year) then 1
		       else 0
               end) as div_ties,
		   ...
		   sum(case
		       when t.owner = g.away_owner and winner = "Away" and g.away_proj < g.home_proj and g.week < 15 then 1
               when t.owner = g.home_owner and winner = "Home" and g.home_proj < g.away_proj and g.week < 15 then 1
               else 0
               end) as upset_wins,
	       sum(case
		       when t.owner = g.away_owner and winner = "Home" and g.away_proj > g.home_proj and g.week < 15 then 1
               when t.owner = g.home_owner and winner = "Away" and g.home_proj > g.away_proj and g.week < 15 then 1
               else 0
               end) as upset_losses,
	       sum(case
	       ...
	       sum(case
		       when t.owner = g.away_owner and g.week < 15 then g.away_score
               when t.owner = g.home_owner and g.week < 15 then g.home_score
               else 0
               end) as pts_for,
	       sum(case
		       when t.owner = g.away_owner and g.week < 15 then g.home_score
               when t.owner = g.home_owner and g.week < 15 then g.away_score
               else 0
               end) as pts_against,
	       sum(case
		       when t.owner = g.away_owner and g.week < 15 then g.away_score - g.home_score
               when t.owner = g.home_owner and g.week < 15 then g.home_score - g.away_score
               else 0
               end) as pts_diff,
	       round(avg(case
				     when t.owner = g.away_owner and g.week < 15 then g.away_score
